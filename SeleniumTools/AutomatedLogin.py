...
if browser.is_text_present('In what city was your high school?'):
    browser.find_by_id('secretquestion').first.fill('Nazem')

elif browser.is_text_present('In what city was your father born?'):
    browser.find_by_id('secretquestion').first.fill('irbid')
    ...
    browser.find_by_id('secretquestion').first.fill('Choueifat')
